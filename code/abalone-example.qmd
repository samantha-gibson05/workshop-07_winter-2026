...
# make new df from abalone
abalone_clean <- abalone  |>
  # cleaned names w/ underscore + no caps
  clean_names()  |>
...
ggplot(data = abalone_clean, # data is abalone_clean
       aes(x = mean_ph, # x is mena_ph (predictor)
           y = change_in_area)) + # y is change_in_area (response)
...
QQ plot = normality check --> assumption met
residuals vs fitted + scale location = homoscedascity check --> generally follows consistency across values
residuals vs leverage = outliers check --> assumption met


residuals seem homoscedastic (consistent across values) based on residuals vs fitted and scale-location plots. residuals seem normally distrbuted based on the QQ plot, the points mostly follow the reference line, there were also no outliers that might influence model estimates or prediction based on residuals vs leverage.
...
**2.9827**  
with each increas in pH the model predicts an increase of 3.0 +/- change in shell surfface are per day (units)

What is the intercept estimate (with standard error?)  

**-18.5010**
when pH = 0 the model predicts that abalone change in shell surface area per day is -18.5 (units) --> note this is not biologically realistic 
...
**based on the model we could expect the change in area to be 5.36 at pH 8**
